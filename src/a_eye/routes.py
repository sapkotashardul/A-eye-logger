...
def login():
    ...
    if form.validate_on_submit():
        ...
        if study:
            ...
            sequences_1 = Sequences.query.filter_by(author=study).all()
            flash('Your datas has been collected !', 'success')


            mydict =[] 
            name = form.code.data[:8]

            for data in sequences_1 :
                ssdict={}
                ssdict['scene']=data.scene
                ssdict['startingDate']=data.startingDate
                ssdict['endingDate']=data.endingDate
                ssdict['cognitiveLoad']=data.cognitiveLoad
                ssdict['focusPoint']=data.focusPoint
                mydict.append(ssdict)
